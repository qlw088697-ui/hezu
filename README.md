...
- 👥 **租客管理**:每位租客可设置权重(如主卧 1.5、次卧 1);支持**批量粘贴名单**一键添加
...
- 📋 **一键复制**结果为纯文本,🖼️ **结算卡片存为图片**(渐变卡片、彩色净额,发群里更直观)、📊 **导出结算 CSV**(UTF-8 BOM,Excel 直接打开)、📤 **系统原生分享**(移动端),🖨️ **打印/存 PDF** 留档
- 🌍 **中英双语**:首次访问按浏览器语言自动选择,右上角一键切换并记住选择
- 🌙 **深色模式**:跟随系统自动切换,也可手动固定浅色/深色;📱 手机、平板、电脑自适应
- 🗄️ **按月保存历史**(支持只读查看任意月份详情)+ 📈 **月度趋势图(按人彩色堆叠,悬停看每人明细)**、导出 / 导入 JSON
...
- 🔒 **隐私与安全**:纯前端单文件应用,无后端、无登录、无追踪,所有数据只存在你自己的浏览器 `localStorage` 里;所有外部数据入口(导入/分享链接/历史/存储)都经过**数据消毒器**规范化,渲染层全部转义,杜绝注入
...
```bash
node test/test.js   # 运行测试:语法检查 + 分摊/结算/房间算法 + 数据消毒 + 分享链接编解码 + CSV 输出 + 历史复制 + 备份解析 + 快照计算(59 项断言)
