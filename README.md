...
- 📋 **一键复制**结果为纯文本,🖼️ **结算卡片存为图片**(渐变卡片、彩色净额,发群里更直观)、📊 **导出结算 CSV**(UTF-8 BOM,Excel 直接打开),🖨️ **打印/存 PDF** 留档
...
```bash
node test/test.js   # 运行测试:语法检查 + 分摊/结算/房间算法 + 数据消毒 + 分享链接编解码 + CSV 输出 + 历史复制(49 项断言)
